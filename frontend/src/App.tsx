...
function App() {
  ...
  const priorityStats = useMemo(() => {
    const scores = actors.map((a) => Number(a.priority_score || 0));
    const average = scores.length ? scores.reduce((a, b) => a + b, 0) / scores.length : 0;
    const urgent = scores.filter((s) => s >= 70).length;
    return { average, urgent };
  }, [actors]);
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-status">
          ...
          <div><strong>System online</strong><small>Evidence correlation engine ready</small></div>
        </div>
        <div className="sidebar-bottom">
          <div className="sidebar-meta"><Activity size={13} /><span>LIVE WORKSPACE</span></div>
          <button className="logout-button" onClick={handleLogout}><LogOut size={15} /><span>Sign out</span></button>
        </div>
      </aside>
            ...
            <>
              <header className="page-header dashboard-header">
                <div>
                  ...
                  <p>Prioritize actors by risk, evidence strength, recency and cross-source correlation.</p>
                </div>
                <button className="primary-button" onClick={() => setPage("search")}><Search size={15} /> Start Investigation</button>
              </header>
              ...
              {loading ? <div className="loading">Loading intelligence data…</div> : (
                <>
                  <section className="stats-grid">
                    ...
                    <div className="stat-card stat-purple"><span>High Risk</span><strong>{highRiskCount}</strong><small>High or critical category</small><ShieldCheck size={18} /></div>
                  </section>

                  <section className="dashboard-grid">
                    <div className="panel actors-panel">
                      <div className="panel-header">
                        <div><div className="eyebrow">PRIORITY QUEUE</div><h3>Actors requiring attention</h3></div>
                        <button className="secondary-button" onClick={() => setPage("search")}>View all <span aria-hidden="true">→</span></button>
                      </div>
                      <div className="table-container">
                        <table>
                          <thead><tr><th>Actor</th><th>Primary handle</th><th>Risk</th><th>Priority</th><th>Confidence</th><th>Last active</th></tr></thead>
                          <tbody>
                            {actors.slice(0, 10).map((actor) => {
                              const score = Number(actor.priority_score || 0);
                              return <tr key={actor.actor_id} className="clickable-row" onClick={() => openActor(actor.actor_id)}>
                                ...
                                <td><span className={`priority-pill ${priorityClass(score)}`}><span>{score}</span> / 100</span></td>
                    ...
                    <aside className="priority-panel">
                      <div className="priority-panel-top"><div><div className="eyebrow">TRIAGE MODEL</div><h3>Priority score</h3></div><BarChart3 size={18} /></div>
                      <div className="priority-score-big">{priorityStats.average.toFixed(0)}<span>/100</span></div>
                      <p>Transparent operational ranking. This is a triage score, not a probability of identity.</p>
                      <div className="priority-legend">
                        <div><span className="legend-swatch critical" />Critical <b>85–100</b></div>
                        <div><span className="legend-swatch high" />High <b>70–84</b></div>
                        <div><span className="legend-swatch medium" />Medium <b>50–69</b></div>
                        <div><span className="legend-swatch low" />Low <b>0–49</b></div>
                      </div>
                    </aside>
                  </section>
                </>
